...
const model = process.env.OLLAMA_MODEL || "deepseek-r1:8b"; // Certifique-se que seu modelo suporta o formato <think>
const ollamaBaseUrl = process.env.OLLAMA_BASE_URL || "http://localhost:11434";
const dataFile = process.env.DATA_FILE || "data.txt";
...
async function setupRAG() {
  ...
  const ollama = new Ollama({ model: model, baseUrl: ollamaBaseUrl });
...
io.on("connection", (socket) => {
  ...
  socket.on("chat message", async (msg) => {
    if (!ragChain) {
      return socket.emit(
        "error message",
        "A IA não está pronta. Tente novamente."
      );
    }
      ...
      for await (const chunk of stream) {
        ...
        if (inThinkBlock && !thinkBlockClosed) {
          if (buffer.includes("</think>")) {
            const thinkContent = buffer.substring(
              0,
  ...
  socket.on("add training text", async (text) => {
    if (!text || text.trim() === "") {
      return socket.emit("training status", {
        message: "Erro: O texto não pode estar vazio.",
        error: true,
      });
    }
    try {
      socket.emit("training status", {
        message: "Recebido. Adicionando ao arquivo de conhecimento...",
      });

      fs.appendFileSync(dataFile, `\n\n${text}`);

      socket.emit("training status", {
        message: `Informação adicionada. Atualizando a base de conhecimento...`,
      });

      await setupRAG();

      socket.emit("training status", {
        message: "Sucesso! A base de conhecimento foi atualizada.",
        success: true,
      });
    } catch (error) {
      console.error("Erro durante o treinamento:", error);
      socket.emit("training status", {
        message: "Ocorreu um erro ao atualizar a base de conhecimento.",
        error: true,
      });
    }
  });
